...
app.service('parserService', function ($http, $log) {

    this.sieToJson = function (f) { 
        return parser.parse(f);
    }

    this.jsonToSie = function (f) {
        var file = "#";
        _.each(f, function (p) {
    ...
    var parser = {
        ...
        _addAttr: function (obj, attr, tokens, pos) {
            if (pos < tokens.length) {
                if (attr == 'belopp') {
                    obj[attr] = parseFloat(tokens[pos].value);
                }else {
